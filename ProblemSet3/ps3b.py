...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        # TODO
        if (all(self.isResistantTo(d) for d in activeDrugs) and self.maxBirthProb * (1- popDensity) >= random.random()):
            resistances = {k:v if random.random() <= (1 - self.mutProb) else not v for k, v in self.resistances.items()}
